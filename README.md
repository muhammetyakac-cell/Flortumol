...
- Admin için hızlı yanıt chip'leri ve opsiyonel AI öneri butonu (server-side proxy `/api/ai-suggest`)
...
1. Proje kökündeki `.env` dosyasında şu değişkenleri güncelle:
   ...
   - `SUPABASE_SERVICE_ROLE_KEY` (admin auth API için)
   - `ADMIN_PASSWORD` (admin girişi, server-side)
   - `ADMIN_PASSWORD2` (opsiyonel, server-side)
   - `OPENAI_API_KEY` (opsiyonel, AI önerileri için, server-side)
2. `supabase/migration_auth_fix.sql` dosyasını Supabase SQL Editor'de çalıştır. (Mevcut üyeleri Supabase Auth'a taşır, RLS backdoor'larını kapatır)
3. `supabase/schema.sql` dosyasını SQL Editor'de çalıştır. (Eski şemalardan migration + storage bucket/policy kurulumu yapar, tekrar çalıştırılabilir)
4. Uygulamayı başlat:
   - `npm install`
   - `npm run dev`
...
## Güvenlik
- Auth: Supabase Auth (JWT) ile güvenli oturum. `auth.uid()` RLS'de çalışır.
- Admin şifreleri ve API key'leri **server-side** (`api/` klasörü) üzerinden doğrulanır, frontend'e derlenmez.
- RLS: Tüm tablolarda `authenticated` + owner/admin scoped politikalar aktiftir. Anon doğrudan erişemez.
- Mevcut üyeler `supabase/migration_auth_fix.sql` ile `auth.users`'a taşınır.
...
- Environment Variables:
  ...
  - `SUPABASE_URL`
  - `ADMIN_PASSWORD`
  - `ADMIN_PASSWORD2` (opsiyonel)
  - `OPENAI_API_KEY` (opsiyonel, AI önerileri için)
  - `STRIPE_SECRET_KEY`
  - `STRIPE_WEBHOOK_SECRET`
