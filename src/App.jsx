  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans selection:bg-fuchsia-500/30">
        ...
        <div className="max-w-[1440px] mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 border-l pl-4 border-slate-200/20">
               {!loggedIn && (
                <button onClick={() => { setMode(mode === 'user' ? 'admin' : 'user'); setShowAuthModal(true); }} className={`text-sm font-semibold underline underline-offset-4 transition-colors ${isAdmin ? 'text-slate-300 hover:text-white' : 'text-slate-500 hover:text-slate-900'}`}>
      ...
      <main className="flex-1 flex flex-col max-w-[1440px] w-full mx-auto p-4 md:p-6">
        ...
        {!loggedIn ? (
          <>
            {/* ---- PUBLIC LANDING PAGE (SEO İÇERİĞİ) ---- */}
            ...
            {showAuthModal && (
              ...
              >
                <div className="w-full max-w-[900px] grid md:grid-cols-2 bg-white rounded-[2rem] shadow-2xl overflow-hidden border border-slate-200 animate-fade-in">

                  {/* Modal Left: Form */}
                  <div className="p-8 md:p-10 flex flex-col justify-center">
                    <button
                      onClick={() => setShowAuthModal(false)}
                      className="self-end mb-4 text-slate-400 hover:text-slate-700 text-2xl leading-none"
                      aria-label="Kapat"
                    >✕</button>
                    <div className="mb-6">
                      ...
                      <p className="text-slate-500 mt-1 font-medium text-sm">Lütfen devam etmek için giriş yapın.</p>
                    </div>
                    ...
                    <div className="space-y-3">
                      <input
                        ...
                        className="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-medium focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all disabled:opacity-50"
                      />
                      <input
                        type="password"
                        ...
                        className="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-medium focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all"
                      />
                    </div>

                    <div className="mt-6 space-y-3">
                      <button onClick={handleSignIn} disabled={loading} className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3.5 rounded-2xl shadow-lg shadow-slate-900/20 transition-transform active:scale-[0.98]">
                        {loading ? 'İşleniyor...' : 'Giriş Yap'}
                      </button>
                      {mode !== 'admin' && (
                        <button onClick={handleSignUp} disabled={loading} className="w-full bg-white hover:bg-slate-50 text-slate-900 border border-slate-200 font-bold py-3.5 rounded-2xl transition-transform active:scale-[0.98]">
                          Yeni Hesap Oluştur
                        </button>
                      )}
                    </div>
